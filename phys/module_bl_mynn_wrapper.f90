...
SUBROUTINE mynnedmf_wrapper_run(                 &
   ...
   do j = jts,jtf
      ...
      counter=0;
              ...
              CALL  mynn_bl_driver(                                    &
     &        initflag,restart,cycling,delt,dz(:,:,j),dx,znt(:,j),     &
     &        u(:,:,j),v(:,:,j),w(:,:,j),th(:,:,j),sqv,sqc,            &
     &        sqi,sqs,qnc2d,qni2d,qnwfa2d,qnifa2d,qnbca2d,ozone(:,:,j),&
     &        p(:,:,j),exner(:,:,j),rho(:,:,j),t3d(:,:,j),xland(:,j),  &
     &        ts(:,j),qsfc(:,j),ps(:,j),ust(:,j),ch(:,j),hfx(:,j),     &
     &        qfx(:,j),rmol(:,j),wspd(:,j),uoce(:,j),voce(:,j),        & 
     &        QKE(:,:,j),qke_adv(:,:,j),Sh3d(:,:,j),Sm3d(:,:,j),       & 
     &        nchem,kdvel,ndvel,chem,vd,frp_mean,emis_ant_no,          &
     &        mix_chem,enh_mix,rrfs_sd,smoke_dbg,                      & 
     &        tsq(:,:,j),qsq(:,:,j),cov(:,:,j),RUBLTEN(:,:,j),         &
     &        RVBLTEN(:,:,j),RTHBLTEN(:,:,j),RQVBLTEN(:,:,j),          & 
     &        rqcblten(:,:,j),rqiblten(:,:,j),rqncblten(:,:,j),        &
     &        rqniblten(:,:,j),ikzero,RQNWFABLTEN(:,:,j),              & 
     &        RQNIFABLTEN(:,:,j),RQNBCABLTEN(:,:,j),rO3blten(:,:,j),   & 
     &        exch_h(:,:,j),exch_m(:,:,j),pblh(:,j),KPBL(:,j),         & 
     &        el_pbl(:,:,j),dqke2d,qWT2d,qSHEAR2d,qBUOY2d,qDISS2d,     & 
     &        qc_bl2d,qi_bl2d,cldfra_bl2d,bl_mynn_tkeadvect,           & 
     &        tke_budget,bl_mynn_cloudpdf,bl_mynn_mixlength,           & 
     &        icloud_bl,bl_mynn_closure,bl_mynn_edmf,bl_mynn_edmf_mom, & 
     &        bl_mynn_edmf_tke,bl_mynn_mixscalars,bl_mynn_output,      & 
     &        bl_mynn_cloudmix,bl_mynn_mixqt,edmf_a2d,edmf_w2d,        & 
     &        edmf_qt2d,edmf_thl2d,edmf_ent2d,edmf_qc2d,sub_thl2d,     &
     &        sub_sqv2d,det_thl2d,det_sqv2d,maxwidth(:,j),maxMF(:,j),  & 
     &        ztop_plume(:,j),ktop_plume(:,j),                         & 
     &        spp_pbl,pattern_spp_pbl2d,rthraten(:,:,j),flag_qi,       &
     &        flag_qni,flag_qs,flag_qc,flag_qnc,FLAG_QNWFA,FLAG_QNIFA, & 
     &        FLAG_QNBCA,flag_ozone,ids,ide,jds,jde,kds,kde,           & 
     &        ims,ime,jms,jme,kms,kme,its,itf,jts,jtf,kts,kte,counter)   
