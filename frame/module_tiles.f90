...
'set_tiles_once: zone out of range, increase MAX_TILE_ZONES in module_domain_type')
  ...
  SUBROUTINE set_tiles2 ( grid , ids , ide , jds , jde , ips , ipe , jps , jpe )
     ...
     IF( ips < smx )THEN;WRITE(mess,'(3A4)')'ips','<','smx';CALL wrf_error_fatal3("<stdin>",249,&
mess);ENDIF
     IF( ipe > emx )THEN;WRITE(mess,'(3A4)')'ipe','>','emx';CALL wrf_error_fatal3("<stdin>",251,&
mess);ENDIF
     IF( jps < smy )THEN;WRITE(mess,'(3A4)')'jps','<','smy';CALL wrf_error_fatal3("<stdin>",253,&
mess);ENDIF
     IF( jpe > emy )THEN;WRITE(mess,'(3A4)')'jpe','>','emy';CALL wrf_error_fatal3("<stdin>",255,&
mess);ENDIF
